		...
		public static Dictionary<string, string> Parse(string[] args)
		{
			...
			for (int i = 0; i < args.Length; i++)
			{
                if (args[i].StartsWith("--"))
                {
                    ...
                    System.Console.WriteLine("using the first add thingy");
                }
                else if (args[i].StartsWith("-"))
                {
                    System.Console.WriteLine(string.Format("adding [{0}]  [{1}]", args[i], args[i + 1]));
                    parsedArgs.Add(args[i++], args[i]);
                }
                else
                {
                    System.Console.WriteLine("using the second add thingy");
